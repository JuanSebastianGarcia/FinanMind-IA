import { BudgetReviewDialog } from "./dialogs/budget_review_dialog.js";
...
export class BudgetPanel {
  ...
  async _handleOpenReview() {
    const result = await new BudgetReviewDialog(this._modal, this._toast, this._api).show();
    if (result && result.changed > 0) await this.refresh();
  }
